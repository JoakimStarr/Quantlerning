...
COURSES = [
    ...
    },
    {
        ...
        "status": "completed",
        "weeks": "3~4 周",
        "intro": "进入公式之前，先回到 2024 年的贵州茅台：年初 1685 元、年末 1524 元，全年复利累计 -6.24%，中间还经历一次 -27.28% 的最大回撤。这只股票是第一章贯穿全章的主线。本章要回答：市场里的钱是怎么定价的？路径分四步——先搞清数据（导论 → 数据获取），再搭起金融学理论骨架（时间价值 → 有效市场），然后落到真实市场指标（绩效 → 基本面），最后产出一份个股分析。上一章给了你数学与编程的工具箱，这一章用它读懂市场；到第二章，再用这些概念去构建与回测策略。",
        "lessons": [
            # 导论：先建立「市场是什么」的地图（资产/参与者/交易机制），再进入数据与理论
    ...
    },
    {
        ...
        "status": "completed",
        "weeks": "3~4 周",
        "intro": "量化到底是什么？第二章从回答这个问题开始。全章走一条完整的策略闭环：策略三要素（信号→持仓→交易）→ 三大策略家族（趋势/均值回归/动量）→ 回测（怎么做、有哪些坑）→ 绩效评估（四类指标与横向对比）→ 交易成本 → 独立完成一份回测报告。学完你能自己写一个策略、在真实数据上回测，并判断结果靠不靠得住。",
        "lessons": [
            {"id": "p2-l0", "title": "前言：量化是什么", "concepts": ["量化定义", "信号→持仓→交易"]},
            ...
            {"id": "p2-l5", "title": "回测：引擎与陷阱", "concepts": ["回测口径", "前视偏差", "过拟合", "多重检验"]},
            {"id": "p2-l6", "title": "绩效指标与策略评估", "concepts": ["夏普", "回撤", "胜率", "横向对比"]},
            {"id": "p2-l7", "title": "交易成本", "concepts": ["佣金", "滑点", "冲击成本"]},
            {"id": "p2-l8", "title": "阶段产出：独立回测", "concepts": ["完整回测报告"]},
        ],
    },
    {
        "phase": 3,
