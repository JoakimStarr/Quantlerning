...
const registry = {
  ...
  macro_series: def('CPI/PPI 真实序列', 'quantlab 库月度 CPI/PPI 同比，双线对比', () => import('../simulators/MacroSeries.vue')),
  ...
  pe_distribution: def('全市场 PE 分布', '真实 quantlab 数据快照 + 中位数/90 分位', () => import('../simulators/PeDistribution.vue')),
  ...
  factor_backtest_dashboard: def('多因子回测仪表盘', 'QuantLab 真实因子回测净值对比', () => import('../simulators/FactorBacktestDashboard.vue')),
  ...
  financial_trends: def('财务比率趋势', '营收/净利柱状 + 可切换比率指标（真实 quantlab 财务数据）', () => import('../simulators/FinancialTrends.vue')),
  dupont_compare: def('三公司杜邦对比', '净利率/周转/乘数/ROE 真实杜邦分解对比（茅台/五粮液/招行）', () => import('../simulators/DuPontCompare.vue')),
  pe_band: def('PE 历史分位带', '个股 PE(TTM) 历史区间 + 均值带 + 当前分位（真实 quantlab 日线）', () => import('../simulators/PeBand.vue')),
  profit_cash_compare: def('利润 vs 现金流对比', '净利润与经营现金流逐年柱状对比 + 净现比（真实 quantlab 财务数据）', () => import('../simulators/ProfitCashCompare.vue')),
  moat_compare: def('护城河宽窄对比', '两家公司 ROE/毛利率十年双线对比（真实 quantlab 财务数据）', () => import('../simulators/MoatCompare.vue')),
